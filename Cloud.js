...
class Cloud extends Group {
  ...
  updateCloudStateArray() {
    ...
    if (this.cloudStateArray.length > this.segments) {
      this.cloudStateArray.splice(
        0,
        this.cloudStateArray.length - this.segments
      );
    } else {
  ...
  updateCloud() {
    ...
    // console.log(
    //   // `Updating cloud at ${new Date().toISOString()}: position=${translation.toString()}, scale=${scale.toString()}`
    // );
    ...
    cloudStateArray.forEach((cloud, index) => {
      ...
      const distributed =
        distribute == null ? void 0 : distribute(cloud, index);
      if (distributed || segments > 1) {
        var _distributed$point;
        cloud.position.copy(cloud.bounds).multiply(
          (_distributed$point =
            distributed == null ? void 0 : distributed.point) !== null &&
            _distributed$point !== void 0
            ? _distributed$point
            : {
                x: random() * 2 - 1,
                y: random() * 2 - 1,
      ...
      cloud.volume =
        ((distributed == null ? void 0 : distributed.volume) !== undefined
          ? distributed.volume
          : Math.max(
